...
if __name__ == "__main__":
    ...
    box, pos = make_bravais2d(N_LATT, **MONOCLINIC_CELL_PARAMS)
    print(f"N: {len(pos)}")
    ...
    # 8x8 lattice -> 64 points. Image is 103x128
    print(im.shape)
    # Units are arbitrary: let's call it  1px = 1 u
    # Kernel has same aspect ratio but less points: how big should it be?
    # N_k/N_l * px_l = 3/8 * 128
    ...
    kernel = gkern(5, sigma=SIG_MATCH)
    im_gauss = sp.signal.convolve(im, kernel, mode=CONVOLVE_MODE)
    ax[1, 0].imshow(im_gauss, aspect="equal")
    ax[1, 0].set(xticks=[], yticks=[], title=f"Convolved with Gaussian (σ={SIG_MATCH})")
